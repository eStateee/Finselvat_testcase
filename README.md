...
Две информационные системы обмениваются юридически значимыми документами:
- Система А (внешняя) — инициирует запросы и отправляет документы
- Система Б (центральный реестр) — хранит документы и обеспечивает их
целостность
Вам необходимо разработать REST API, эмулирующее работу Системы Б. Все документы
подписываются электронной цифровой подписью (ЭЦП) и могут храниться в блокчейн-
подобной структуре (для простоты задания эмуляция хранения в памяти или БД).
