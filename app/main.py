...
def _now_utc_str() -> str:
    """Текущее UTC время в формате ISO-8601 с Z."""
    return datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")


def _build_signed_api_response(transactions: list, count: int) -> dict:
    """Упаковывает список транзакций в SignedApiData для ответа."""
    tx_data_dict = {"Transactions": transactions, "Count": count}
    ...
    return {"Data": data_b64, "Sign": sign_b64, "SignerCert": signer_cert_b64}
...
async def lifespan(app: FastAPI):
    ...
    if get_transaction_count() == 0:
        logger.info("База данных пуста — создаём seed-транзакцию 201 (гарантия).")
        ...
        payload_b64 = base64.b64encode(
            json.dumps(payload_json, ensure_ascii=False, separators=(',', ':')).encode('utf-8')
        ).decode('utf-8')
        ...
        logger.info("Seed-транзакция 201 успешно создана.")
...
async def json_decode_exception_handler(request: Request, exc: json.JSONDecodeError):
    logger.warning("JSONDecodeError on %s", request.url)
    return JSONResponse(
        status_code=status.HTTP_400_BAD_REQUEST,
...
async def messages_outgoing(payload: SignedApiData = Body(...)):
    try:
        data_bytes = base64.b64decode(payload.Data, validate=True)
    except Exception as exc:
        raise ValueError(f"Cannot decode Data as Base64: {exc}") from exc

    try:
        search_request_dict = json.loads(data_bytes.decode('utf-8'))
    except json.JSONDecodeError as exc:
        raise ValueError(f"Cannot parse Data as JSON: {exc}") from exc

    try:
        search_req = SearchRequest(**search_request_dict)
    except Exception as exc:
        raise ValueError(f"Invalid SearchRequest: {exc}") from exc
    ...
    return JSONResponse(
        status_code=200,
        content=_build_signed_api_response(transactions, count_total),
    )
...
async def messages_incoming(payload: SignedApiData = Body(...)):
    ...
    try:
        txs_data_dict = json.loads(data_bytes.decode('utf-8'))
    except json.JSONDecodeError as exc:
        raise ValueError(f"Cannot parse Data as JSON: {exc}") from exc
    ...
    try:
        ...
        raise ValueError(f"Invalid TransactionsData: {exc}") from exc
    ...
    for t in txs_data.Transactions:
        ...
        try:
            msg_bytes = base64.b64decode(t.Data, validate=True)
        except Exception as exc:
            raise ValueError(f"Cannot decode Transaction.Data as Base64: {exc}") from exc

        try:
            msg_dict = json.loads(msg_bytes.decode('utf-8'))
        except json.JSONDecodeError as exc:
            raise ValueError(f"Cannot parse Transaction.Data as JSON: {exc}") from exc

        try:
            msg = Message(**msg_dict)
        except Exception as exc:
            raise ValueError(f"Invalid Message: {exc}") from exc
        ...
        if msg.InfoMessageType != 215:
            try:
                payload_bytes = base64.b64decode(msg.Data, validate=True)
                payload_dict = json.loads(payload_bytes.decode('utf-8'))
            except Exception as exc:
                raise ValueError(f"Cannot decode/parse Message.Data: {exc}") from exc
        ...
        if is_new and msg.InfoMessageType in (202, 203):
            ...
            receipt_payload_b64 = base64.b64encode(
                json.dumps({"BankGuaranteeHash": bg_hash}, ensure_ascii=False, separators=(',', ':')).encode('utf-8')
            ).decode('utf-8')
            ...
            rec_msg_b64 = base64.b64encode(
                canonical_json_message(rec_msg_dict).encode('utf-8')
            ).decode('utf-8')
            ...
            logger.info("Сгенерирован квиток 215 для транзакции %s", t.Hash)

    return JSONResponse(
        status_code=200,
        content=_build_signed_api_response(receipts, len(receipts)),
    )
